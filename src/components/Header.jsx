...
const Header = () => {
  ...
  const menuOpacitySpring = useSpring({
    ref: menuOpacitySprintRef,
    to: { opacity: isOpen ? 1 : 0, display: isOpen ? "block" : "none" },
  });
  ...
  useChain(
    isOpen
      ? [menuOpacitySprintRef, trailSpringRef]
      : [trailSpringRef, menuOpacitySprintRef],
    isOpen ? [0, 0] : [0, 0.25]
  );
  return (
    <div className="flex justify-between items-center px-10 py-5 w-screen fixed top-0 bg-white bg-opacity-90 z-50">
      ...
      <button className="md:hidden">
        ...
        <animated.div
          style={menuOpacitySpring}
          className="w-screen h-screen absolute left-0 top-0 bg-white z-10"
        >
